...
document.querySelector("#history").addEventListener("click", () => {
  ...
  for (let i = info.customer_1.balanceHistory.length - 1; i >= 0; i--) {
    ...
    frame.History.main[`main_${i}`].Edate.textContent =
      info.customer_1.balanceHistory[i].date;
    frame.History.main[`main_${i}`].appendChild(
      frame.History.main[`main_${i}`].Edate
    );

    frame.History.main[`main_${i}`].Etype = document.createElement("h4");
    frame.History.main[`main_${i}`].Etype.textContent =
      info.customer_1.balanceHistory[i].type;
    frame.History.main[`main_${i}`].appendChild(
      frame.History.main[`main_${i}`].Etype
    );

    frame.History.main[`main_${i}`].Eammount = document.createElement("h4");
    frame.History.main[`main_${i}`].Eammount.textContent =
      info.customer_1.balanceHistory[i].ammount;
    frame.History.main[`main_${i}`].appendChild(
      frame.History.main[`main_${i}`].Eammount
    );

    frame.History.main[`main_${i}`].Etime = document.createElement("h4");
    frame.History.main[`main_${i}`].Etime.textContent =
      info.customer_1.balanceHistory[i].time;
    frame.History.main[`main_${i}`].appendChild(
      frame.History.main[`main_${i}`].Etime
    );
