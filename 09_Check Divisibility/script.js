...
btnSubmitScreen_1.addEventListener("click", () => {
  ...
  for (let i = 0; i < inLenValue; i++) {
    ...
    document
      .querySelector("#input-block-in-screen-2")
      .appendChild(inScreen_2Value[i][`divTag_${i}`]);

    // input tag under the div tag
    inScreen_2Value[i][`divTag_${i}`][`inTag_${i}`] =
      document.createElement("input");
    inScreen_2Value[i][`divTag_${i}`][`inTag_${i}`].className =
      "input-tag-screen-2";
    inScreen_2Value[i][`divTag_${i}`][
      `inTag_${i}`
    ].idName = `input-screen-2-id-${i + 1}`;

    inScreen_2Value[i][`divTag_${i}`][`inTag_${i}`].setAttribute(
      "type",
      "number"
    );
    inScreen_2Value[i][`divTag_${i}`][`inTag_${i}`].setAttribute(
    ...
    inScreen_2Value[i][`divTag_${i}`].appendChild(
      inScreen_2Value[i][`divTag_${i}`][`inTag_${i}`]
    );

    // span tag under the div tag for required star
    inScreen_2Value[i][`divTag_${i}`][`spanTag_${i}`] =
      document.createElement("span");
    inScreen_2Value[i][`divTag_${i}`][`spanTag_${i}`].className =
      "required-star";
    ...
    inScreen_2Value[i][`divTag_${i}`][`inTag_${i}`].addEventListener(
      "input",
      (e) => {
        inScreen_2ValueArray[i] = e.target.value;
        // this is for red star
        if (e.target.value != "") {
          inScreen_2Value[i][`divTag_${i}`][`spanTag_${i}`].style.display =
            "none";
        } else {
          inScreen_2Value[i][`divTag_${i}`][`spanTag_${i}`].style.display =
            "block";
  ...
  if (inLenValue == 1) {
    ...
    // first last
    inScreen_2Value[inLenValue - 1][
      `divTag_${inLenValue - 1}`
    ...
    // second last
    inScreen_2Value[inLenValue - 2][
      `divTag_${inLenValue - 2}`
  ...
  document
    .querySelector("#submit-block-in-screen-2")
    .appendChild(btnSubmitScreen_2);
  ...
  btnSubmitScreen_2.addEventListener("click", () => {
    ...
    for (let i of inScreen_2ValueArray) {
      ...
      if (i % 3 == 0 && i % 2 != 0) {
        ...
        document
          .querySelector("#div-op-screen")
          .appendChild(opScreenValue[tempCount]);
        tempCount++;
      }
    }
    ...
    if (tempCount == 1) {
      document.querySelector("#div-op-screen").style.gridTemplateColumns =
        "auto";
    } else if (tempCount == 2) {
      document.querySelector("#div-op-screen").style.gridTemplateColumns =
        "auto auto";
