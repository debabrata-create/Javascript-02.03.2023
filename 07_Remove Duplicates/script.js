...
function addItem(itemId) {
  ...
  itemsRow[count].row.innerHTML = `<p class="sr-no">${
    count + 1
  }</p>  <p class="pro-name">${
    itemsInfo[itemId].proName
  }</p>  <p class="pro-price">${itemsInfo[itemId].proPrice}</p>`;
  ...
  document.querySelector("#TA-s").textContent =
    strAmmount + withDupliAmmount + "$";
...
function removeDupliItem() {
  ...
  for (let key of keysArrayObjItems) {
    ...
    itemsDupliRow[dupliCount].row.innerHTML = `<p class="sr-no">${
      dupliCount + 1
    }</p>  <p class="pro-name">${
      objItems[key].proName
    }</p>  <p class="pro-price">${objItems[key].proPrice}</p>`;

    document
      .querySelector("#item-list-duplicate")
      .appendChild(itemsDupliRow[dupliCount].row);
    line[dupliCount] = document.createElement("hr");
    document
      .querySelector("#item-list-duplicate")
      .appendChild(line[dupliCount]);

    // footer updation
    withoutDupliAmmount = withoutDupliAmmount + Number(objItems[key].proPrice);
    document.querySelector("#TQ-d").textContent =
      strQuantity + (dupliCount + 1);
    document.querySelector("#TA-d").textContent =
      strAmmount + withoutDupliAmmount + "$";
