...
function addItem(id) {
  ...
  if (leftRow[id] != undefined) {
    leftRow[id].quantity.textContent =
      Number(leftRow[id].quantity.textContent) + 0.5;
    leftRow[id].price.textContent = `${itemsInfo[id].itemPrice} * ${
      leftRow[id].quantity.textContent
    } = ${
      Number(itemsInfo[id].itemPrice) * Number(leftRow[id].quantity.textContent)
    }`;
    ...
    document.querySelector("#ta-left").textContent =
      taText + leftTotalAmmount * 0.5;
...
document.querySelector("#btn-correct-bug").addEventListener("click", () => {
  ...
  for (let i = 0; i < 8; i++) {
    if (leftRow[i] != undefined) {
      ...
      rightRow[i].quantity.textContent =
        Number(leftRow[i].quantity.textContent) * 2;
      rightRow[i].appendChild(rightRow[i].quantity);

      // 4 -> price
      rightRow[i].price = document.createElement("h4");
      rightRow[i].price.textContent = `${leftRowPriceArray[i]} * ${
        rightRow[i].quantity.textContent
      } = ${leftRowPriceArray[i] * Number(rightRow[i].quantity.textContent)}`;
  ...
  document.querySelector("#tq-right").textContent =
    tqText + leftTotalQuantity * 2;
